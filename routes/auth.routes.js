...
router.post("/signup", isLoggedOut, (req, res, next) => {
  ...
  if (!username || !email || !password) {
    res.render("auth/signup", { errMessage: "All fields are mandatory.Please fill them out." });
    return;
  }
  //Force user to choose strong PW
  if (!pwRegEx.test(password)) {
    res.status(500).render("auth/signup", {
      errMessage: "Password needs to have at least 6 chars and must contain at least one number, one lowercase and one uppercase letter.",
    });
  }
    ...
    bcryptjs
      ...
      .catch(err => {
        ...
        if (err instanceof mongoose.Error.ValidationError) {
          ...
          res.status(500).render("auth/signup", { errMessage: "Either username or email is already used. Must be unique." });
        } else {
